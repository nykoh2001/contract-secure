...
def inst_log0(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"LOG0 instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_log1(instruction: SSAInstruction, state: State):
    ...
    if args_len != 3:
        logger.error(
            f"LOG1 instruction needs 3 arguments but {args_len} was given")
        raise Exception
...
def inst_log2(instruction: SSAInstruction, state: State):
    ...
    if args_len != 4:
        logger.error(
            f"LOG2 instruction needs 4 arguments but {args_len} was given")
        raise Exception
...
def inst_log3(instruction: SSAInstruction, state: State):
    ...
    if args_len != 5:
        logger.error(
            f"LOG3 instruction needs 5 arguments but {args_len} was given")
        raise Exception
...
def inst_log4(instruction: SSAInstruction, state: State):
    ...
    if args_len != 6:
        logger.error(
            f"LOG4 instruction needs 6 arguments but {args_len} was given")
        raise Exception
