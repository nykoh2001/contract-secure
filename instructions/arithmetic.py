...
def inst_add(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"ADD instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_mul(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"MUL instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_sub(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"SUB instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_div(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"DIV instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_sdiv(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"SDIV instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_mod(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"MOD instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_smod(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"SMOD instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_addmod(instruction: SSAInstruction, state: State):
    ...
    if args_len != 3:
        logger.error(
            f"ADDMOD instruction needs 3 arguments but {args_len} was given")
        raise Exception
...
def inst_mulmod(instruction: SSAInstruction, state: State):
    ...
    if args_len != 3:
        logger.error(
            f"MULMOD instruction needs 3 arguments but {args_len} was given")
        raise Exception
...
def inst_exp(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"EXP instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_shl(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"SHL instruction needs 2 arguments but {args_len} was given")
        raise Exception
    ...
    registers = state.registers
    shift, value = get_argument_value(
        args, 0, registers), get_argument_value(args, 1, registers)
...
def inst_shr(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"SHR instruction needs 2 arguments but {args_len} was given")
        raise Exception
...
def inst_sar(instruction: SSAInstruction, state: State):
    ...
    if args_len != 2:
        logger.error(
            f"SAR instruction needs 2 arguments but {args_len} was given")
        raise Exception
