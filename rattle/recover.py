...
class InternalRecover(object):
    ...
    def recover_loop(self, function: SSAFunction) -> None:
        ...
        for block in function:
            # make a new list because we modify it during iterating
            for insn in list(block.insns):
                ...
                if insn.insn.is_push:
                    ...
                    insn.append_argument(ConcreteStackValue(
                        insn.insn.operand, self.concrete_values))
    ...
    def identify_blocks(self, function: SSAFunction) -> None:
        ...
        insns = list(EVMAsm.disassemble_all(
            binascii.unhexlify(self.filedata), 0))
                ...
                if isinstance(target, ConcreteStackValue):
                    ...
                    def handle_writers(writer, depth) -> bool:
                        ...
                        if writer.insn.is_push:
                            value = cast(ConcreteStackValue,
                                         writer.arguments[0])
                            dirty |= block.add_jump_target(
        ...
        for edge in start_block.in_edges.copy():
            ...
            if start_block in edge.jump_edges:
                ...
                if last_insn.insn.name not in {'JUMP', 'JUMPI'}:
                    logger.warning(
                        f"Unknown branch instruction. Instruction name is {last_insn.insn.name}")
                    continue
    ...
    def split_dispatched_methods(self, function: SSAFunction) -> None:
        ...
        for block in function:
            for insn in block:
                ...
                logger.debug(
                    f"Found calldataload(0) at {insn.offset:#x} {insn}")
                ...
                for user in users:
                    if user.insn.name != 'EQ':
                        logger.warning(
                            f"Found CALLDATALOAD(0) user that isn't an EQ {user}")
                        continue
                    ...
                    for jump in eq_readers:
                        if isinstance(jump, ConditionalInternalCall) or isinstance(jump, InternalCall):
                            if jump.target.hash != hash:
                                method = SSAFunction(
                                    offset=jump.target.offset, name='', hash=hash)
            ...
            for insn in block:
                ...
                for comp in insn.return_value.readers():
                    ...
                    for jump in list(jumps):
                        ...
                        logger.debug(
                            f"_fallthrough function at {jump_target:#x}")

                        fallthrough = self.extract_method(
    ...
    def split_inline_functions(self, function: SSAFunction) -> List[SSAFunction]:
        ...
        for block in function:
            for insn in block:
                ...
                print(
                    f"Inline function identified between {writer.offset:#x} {writer} and {insn.offset:#x} {insn}")
            ...
            for reader in list(item.readers()):
                ...
                if len(reader.arguments) == 2:
                    ...
                    if all([isinstance(x, ConcreteStackValue) for x in reader.arguments]):
                        ...
                        x: int = cast(ConcreteStackValue,
                                      reader.arguments[0]).concrete_value
                        y: int = cast(ConcreteStackValue,
                                      reader.arguments[1]).concrete_value

                        op = two_concrete_arguments.get(reader.insn.name, None)
                        if op is not None:
                            do_replace(ConcreteStackValue(
                                op(x, y), self.concrete_values))
                    else:

                        def one_concrete_argument(symbolic_idx: int, concrete_idx: int) -> None:
                            symbolic_arg: StackValue = reader.arguments[symbolic_idx]
                            y: int = cast(
                                ConcreteStackValue, reader.arguments[concrete_idx]).concrete_value
    ...
    def update_jump_offset_of_phi_instruction(self, phi_instruction: SSAInstruction, old_value, new_value):
        ...
        for phi in phis:
            self.update_jump_offset_of_phi_instruction(
                phi, old_value, new_value)
    ...
    def update_jump_offsets(self, jump_instruction: SSAInstruction, old_value, new_value):
        ...
        if isinstance(jump_first_arg, ConcreteStackValue):
            jump_instruction.replace_argument(
                jump_first_arg, ConcreteStackValue(new_value, self.concrete_values))
            return

        phi_instruction = jump_first_arg.writer
        self.update_jump_offset_of_phi_instruction(
            phi_instruction, old_value, new_value)
                    ...
                    for prev_block in block.in_edges:
                        ...
                        if prev_block.fallthrough_edge == block:
                            ...
                            self.update_jump_offsets(
                                prev_block.insns[-1], block.offset, next_block.offset)
        ...
        for function in self.functions:
            for block in list(function):
                ...
                if next_block is None:
                    print(
                        "Trying to remove block with None fallthrough! {}".format(block))
                    continue
...
class Recover(object):
    ...
    def __init__(self, filedata: bytes, edges: List[Tuple[int, int]], optimize=False, split_functions=True) -> None:
        self.internal = InternalRecover(
            filedata, edges, optimize, split_functions)
