...
class MethodDispatch(EVMAsm.EVMInstruction):
    ...
    def __init__(self, target: 'SSAFunction', args: int, offset: int = 0) -> None:
        self.target = target
        super().__init__(0x101, "CONDICALL", 0, args, 1,
                         0, "Conditional Internal Call", 0, offset)


class InternalCallOp(EVMAsm.EVMInstruction):
    def __init__(self, args: int, offset: int = 0) -> None:
        super().__init__(0x102, "ICALL", 0, args, 1, 0,
                         "Conditional Internal Call", 0, offset)
        ...
        if len(self.block.in_edges) > 1:
            ...
            for edge in self.block.in_edges:
                ...
                try:
                    ...
                    '''
                    new_slot = PlaceholderStackValue(
                        self.sp + len(edge_stack), list(self.block.in_edges)[0])
            ...
            phi_insn: SSAInstruction = SSAInstruction(
                PHIInstruction(num_args, self, self.block.offset), self.block)
...
class SSAInstruction(object):
    ...
    def remove_argument(self, old: StackValue) -> None:
        ...
        new_phi = PHIInstruction(
            len(new_args), self.insn.original, self.insn.pc)
...
class SSABasicBlock(object):
    ...
    def __init__(self, offset: int, function: 'SSAFunction') -> None:
        ...
        self.insns = []
        self.stack = [PlaceholderStackValue(-x, self)
    ...
    def __repr__(self) -> str:
        ...
        if self.fallthrough_edge:
            out_block: SSABasicBlock = cast(
                SSABasicBlock, self.fallthrough_edge)
        ...
        for block in self:
            for insn in block:
                if insn.insn.name == 'SHA3':
                    ...
                    locations.update(
                        set(range(start.concrete_value, end.concrete_value)))
